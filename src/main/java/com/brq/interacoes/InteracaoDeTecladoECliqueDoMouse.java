...
public interface InteracaoDeTecladoECliqueDoMouse {
	...
	default void moverParaCima() {
		intanciarRobot().keyPress(KeyEvent.VK_UP);
	}

	default void moverParaBaixo() {
		intanciarRobot().keyPress(KeyEvent.VK_DOWN);
	}

	default void moverParaEsquerda() {
		intanciarRobot().keyPress(KeyEvent.VK_LEFT);
	}

	default void moverParaDireita() {
		intanciarRobot().keyPress(KeyEvent.VK_RIGHT);
	}

	default void pressionarEnter() {
		intanciarRobot().keyPress(KeyEvent.VK_ENTER);
	}

	default void pressionarEnter(By by) {
	...
	default void backspace(int quantidade) {
		for (int i = 1; i <= quantidade; i++) {
			intanciarRobot().keyPress(KeyEvent.VK_BACK_SPACE);
		}
	}

	default void delete(int quantidade) {
		for (int i = 1; i <= quantidade; i++) {
			intanciarRobot().keyPress(KeyEvent.VK_DELETE);
		}
	}

	default void espaco(int quantidade) {
		for (int i = 1; i <= quantidade; i++) {
			intanciarRobot().keyPress(KeyEvent.VK_SPACE);
		}
	}

	default void esc() {
		intanciarRobot().keyPress(KeyEvent.VK_ESCAPE);
	}

	default void control() {
		intanciarRobot().keyPress(KeyEvent.VK_CONTROL);
	}

	default void shift() {
		intanciarRobot().keyPress(KeyEvent.VK_SHIFT);
	}

	default void alt() {
		intanciarRobot().keyPress(KeyEvent.VK_ALT);
	}

	default void pageUp() {
		intanciarRobot().keyPress(KeyEvent.VK_PAGE_UP);
	}

	default void pageDown() {
		intanciarRobot().keyPress(KeyEvent.VK_PAGE_DOWN);
	}

	default void end() {
		intanciarRobot().keyPress(KeyEvent.VK_END);
	}

	default void home() {
		intanciarRobot().keyPress(KeyEvent.VK_HOME);
	}

	default void windows() {
		intanciarRobot().keyPress(KeyEvent.VK_WINDOWS);
	}

	default void tab() {
		intanciarRobot().keyPress(KeyEvent.VK_TAB);
	}

	default void capsLock() {
		intanciarRobot().keyPress(KeyEvent.VK_CAPS_LOCK);
	}

	default void copiar() {
		intanciarRobot().keyPress(KeyEvent.VK_COPY);
	}

	default void cortar() {
		intanciarRobot().keyPress(KeyEvent.VK_CUT);
	}

	default void colar() {
		intanciarRobot().keyPress(KeyEvent.VK_PASTE);
	}

	default void clicarBotaoEsquerdoMouse(int quantidade) {
		for (int i = 0; i <= quantidade; i++) {
			intanciarRobot().mousePress(MouseEvent.BUTTON1);
		}
	}

	default void clicarBotaoDireitoMouse(int quantidade) {
		for (int i = 0; i <= quantidade; i++) {
			intanciarRobot().mousePress(MouseEvent.BUTTON2);
		}
	}

	default void rolar(int quantidadeRolagem) {
		intanciarRobot().mouseWheel(quantidadeRolagem);
	}

	default void moverParaCoordenada(int coordenadaX, int coordenadaY) {
		intanciarRobot().mouseMove(coordenadaX, coordenadaY);
	}
	...
	default void tipoCaracteres(int... keyCodes) {
